use pos::{Branch, Invoice, PosError, ZatcaStatusInfo};
use pos::AppState;
...
// Derives a key from machine-specific data and encrypts the private key
// before storing in SQLite. Key is unusable if DB is copied to another machine.
// ============================================================
fn get_machine_id() -> String {
    // Combine multiple machine identifiers for derivation
    let hostname = hostname::get()
        .map(|h| h.to_string_lossy().to_string())
        .unwrap_or_else(|_| "pos-default-hostname".to_string());
    let machine_id = std::fs::read_to_string("/etc/machine-id")
        .or_else(|_| std::fs::read_to_string("/var/lib/dbus/machine-id"))
        .unwrap_or_else(|_| "pos-default-machine-id".to_string());

    format!("{}:{}", hostname.trim(), machine_id.trim())
}

fn derive_key_from_machine() -> [u8; 32] {
    use ring::digest::{digest, SHA256};
    let machine_info = get_machine_id();
    let hash = digest(&SHA256, machine_info.as_bytes());
    let mut key = [0u8; 32];
    key.copy_from_slice(hash.as_ref());
    key
}

fn encrypt_for_storage(data: &[u8], machine_key: &[u8; 32]) -> Vec<u8> {
    use ring::aead::{Aad, LessSafeKey, Nonce, UnboundKey, AES_256_GCM};
    let unbound_key = UnboundKey::new(&AES_256_GCM, machine_key)
...
fn decrypt_from_storage(data: &[u8], machine_key: &[u8; 32]) -> Option<Vec<u8>> {
    use ring::aead::{Aad, LessSafeKey, Nonce, UnboundKey, AES_256_GCM};
    if data.len() < 12 {
        return None;
    }
    let unbound_key = UnboundKey::new(&AES_256_GCM, machine_key).ok()?;
    let key = LessSafeKey::new(unbound_key);
    let nonce_arr: [u8; 12] = data[..12].try_into().ok()?;
...
// ============================================================
// ZATCA Environment URLs
// Debug builds use the sandbox; release builds use production.
// ============================================================
...
// Uses rcgen for pure-Rust ECDSA P-256 CSR generation.
// ============================================================
fn generate_csr(private_key: &[u8], branch: &Branch) -> Result<Vec<u8>, String> {
    use rcgen::{CertificateParams, DistinguishedName, DnType, KeyPair};

    // Parse the PKCS8 private key generated by ring
    let key_pair = KeyPair::from_der(private_key)
        .map_err(|e| format!("فشل تحميل المفتاح: {:?}", e))?;

    let mut dn = DistinguishedName::new();
    dn.push(DnType::CommonName, &branch.name_ar);
    dn.push(DnType::OrganizationName, &branch.name_ar);
    dn.push(DnType::CountryName, "SA");

    if let Some(ref cr) = branch.cr_number {
        dn.push(DnType::from_oid(&[2, 5, 4, 5]), cr);
    }

    // ZATCA organizationIdentifier OID 2.5.4.97 — required for e-invoicing
    if let Some(ref vat) = branch.vat_number {
        dn.push(
            DnType::from_oid(&[2, 5, 4, 97]),
            &format!("VATSA-{}", vat),
        );
    }

    if let Some(ref address) = branch.address {
        dn.push(DnType::LocalityName, address);
    }

    let mut params = CertificateParams::default();
    params.distinguished_name = dn;
    params.key_pair = Some(key_pair);
    // ECDSA P-256 is the default for rcgen when given a P-256 key

    let cert = rcgen::Certificate::from_params(params)
        .map_err(|e| format!("فشل إنشاء CSR: {:?}", e))?;

    let csr_pem = cert.serialize_request_pem()
        .map_err(|e| format!("فشل تسلسل CSR: {:?}", e))?;

    log::info!(target: "pos::zatca", "CSR generated for branch={}", branch.name_ar);
...
pub fn generate_and_store_invoice_qr(
    ...
    let _signature = {
        let machine_key = derive_key_from_machine();
        let key_b64: Option<String> = conn.query_row(
            "SELECT value FROM settings WHERE key = 'zatca_private_key_encrypted'",
            [],
        ...
        key_bytes.and_then(|bytes| sign_invoice_hash(xml_hash.as_bytes(), &bytes).ok())
    };
